    ...
    for (uint32_t idx = (right_tuples_idx_ < 0 ? 0 : right_tuples_idx_); idx < right_tuples_.size(); idx++) {
      ...
      if (Matched(&left_tuple_, &right_tuple)) {
        for (uint32_t i = 0; i < left_executor_->GetOutputSchema().GetColumnCount(); i++) {
          val.push_back(left_tuple_.GetValue(&left_executor_->GetOutputSchema(), i));
        }
        for (uint32_t i = 0; i < right_executor_->GetOutputSchema().GetColumnCount(); i++) {
          val.push_back(right_tuple.GetValue(&right_executor_->GetOutputSchema(), i));
        }
        *tuple = Tuple{val, &GetOutputSchema()};
        right_tuples_idx_ = idx + 1;
        return true;
      }
    }

    if (right_tuples_idx_ == -1 && plan_->GetJoinType() == JoinType::LEFT) {
